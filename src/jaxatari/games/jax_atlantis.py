...
class GameConfig:
    ...
    bullet_width: int = 3
    bullet_height: int = 3
    # bullet_x: int = 40  # Fixed x position
    airplane_width: int = 10
    airplane_height: int = 5
    num_lanes: int = 4
    lane_spacing: int = 16
    airplane_speeds = [2.0, 5.0]
    ...
    def __post_init__(self):
        if self.lane_borders is None:
            self.lane_borders = [
                ...
                1 * self.lane_spacing + self.top_border,  # Lane 1
                2 * self.lane_spacing + self.top_border,  # Lane 2
                3 * self.lane_spacing + self.top_border,  # Lane 3
                4 * self.lane_spacing + self.top_border,  # Lane 4
            ]
...
class GameState(NamedTuple):
    ...
    bullets: chex.Array  # Shape: (x, y)
    airplanes: chex.Array  # Shape: (airplane_type, num_lanes, direction, 2) for x,y positions
    buildings: chex.Array  # Shape: (x, y)
    score: chex.Array
...
class SkyObservation(NamedTuple):
    bullet: EntityPosition
    airplane: EntityPosition
    score: jnp.ndarray
...
class JaxAtlantis(JaxEnvironment[GameState, SkyObservation, AtlantisInfo]):
    def __init__(self):
        super().__init__()
        self.config = GameConfig()
        self.state = self.reset()

    def reset(self, key: jax.random.PRNGKey = None) -> Tuple[SkyObservation, GameState]:
        """Initialize a new game state"""
        # Start chicken at bottom
        bullet_y = self.config.bottom_border + self.config.bullet_height - 1
        ...
        for lane in range(self.config.num_lanes):
            lane_y = (
                ...
                - int(self.config.airplane_height / 2)
            )
            # Upper 5 lanes start from right, lower 5 lanes start from left
            if lane < 5:
                x = (
                        self.config.screen_width - self.config.airplane_width + 0
                )  # Start from right
            else:
                x = 0  # Start from left
    ...
    def step(self, state: GameState, action: int) -> tuple[SkyObservation, GameState, float, bool, AtlantisInfo]:
        ...
        new_y = jnp.clip(
            ...
            self.config.bottom_border + self.config.bullet_height - 1,
        )
        ...
        for lane in range(self.config.num_lanes):
            ...
            new_x = jnp.where(
                ...
                jnp.where(
                    new_x > self.config.screen_width, -self.config.airplane_width, new_x
                ),
                jnp.where(
                    new_x < -self.config.airplane_width, self.config.screen_width, new_x
                ),
            )
        ...
        def check_collision(car_pos):
            ...
            return jnp.logical_and(
                self.config.bullet_x < car_x + self.config.airplane_width,
                jnp.logical_and(
                    self.config.bullet_x + self.config.bullet_width > car_x,
                    jnp.logical_and(
                        state.chicken_y - self.config.bullet_height < car_y,
                        state.chicken_y > car_y - self.config.airplane_height,
                    ),
                ),
            )
    ...
    def _get_observation(self, state: GameState):
        ...
        chicken = EntityPosition(
            x=jnp.array(self.config.bullet_x),
            y=state.chicken_y,
            width=jnp.array(self.config.bullet_width),
            height=jnp.array(self.config.bullet_height),
        )
            ...
            cars = cars.at[i].set(
                jnp.array(
                    [
                        ...
                        self.config.airplane_width,  # width
                        self.config.airplane_height,  # height
                    ]
                )
            )
        return SkyObservation(chicken=chicken, car=cars, score=state.score)

    @partial(jax.jit, static_argnums=(0,))
    def _get_info(self, state: GameState) -> AtlantisInfo:
        return AtlantisInfo(time=state.time)
...
class FreewayRenderer(AtraJaxisRenderer):
    ...
    def render(self, state):
        ...
        raster = aj.render_at(raster, 110, self.game_config.bottom_border + self.game_config.bullet_height - 1, chicken_idle)
        ...
        raster = aj.render_at(raster, self.game_config.bullet_x, state.chicken_y, chicken)
